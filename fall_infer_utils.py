...
def draw(img, xscale, yscale, pred):
    ...
    if len(pred):
        for detect in pred:
            ...
            img_ = cv2.rectangle(
                img, (detect[0], detect[1]), (detect[2], detect[3]), (0, 0, 255), 5
            )
    return img_


height, width = 640, 640
ONNX_PATH = "./weights/fall_detect.onnx"
sess = rt.InferenceSession(ONNX_PATH)
input_name = sess.get_inputs()[0].name
label_name = sess.get_outputs()[0].name


def get_fall_down_img(img):
    img0 = img.copy()
    ...
    result = nms(pred, 0.65, 0.45)
    ret_img = draw(img0, x_scale, y_scale, result)
    ret_img = ret_img[:, :, ::-1]
    return ret_img


def get_fall_down_result(img):
    img0 = img.copy()
    x_scale = img0.shape[1] / width
    y_scale = img0.shape[0] / height
    img = img0 / 255.0
    ...
    pred_conf = np.max(pred_class, axis=-1)
    pred = np.insert(pred, 4, pred_conf, axis=-1)
    result = nms(pred, 0.85, 0.45)
    # ret_img = draw(img0, x_scale, y_scale, result)
    # ret_img = ret_img[:, :, ::-1]
    return img0, x_scale, y_scale, result


if __name__ == "__main__":
    video_path = "./images/fall.mp4"
    cap = cv2.VideoCapture(video_path)
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        frame = get_fall_down_img(frame)
